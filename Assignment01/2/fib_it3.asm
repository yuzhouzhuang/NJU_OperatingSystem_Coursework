section .text
  ...
  Fib_Big_Add:
      ...
      push edx
      xor ebx, ebx
      mov eax, fib_cal_temp
      mov ecx, fib_cal_result
      add eax, buffer_len
      add ecx, buffer_len
      dec eax
      dec ecx ; Adjust the location to the last bit
      mov dl, [result_len]
      mov dh, 0
      Big_Add_Cal:
        cmp dh, dl
        je  Big_Add_Finish  ; Finish This
        push edx
        
        mov dh, byte[ecx]
        mov dl, byte[eax]
        
        mov byte[eax], dh ; Store Fx-1
        ...
        mov bl, 0
        cmp dh, 39h
        jle Big_Add_Next;无需进位
       	mov bl, 1
       	sub dh, 10
        Big_Add_Next:
        	mov byte[ecx], dh ; store result
        	pop edx
        	inc dh
        	dec ecx
        	dec eax ; Higher Bits
      ...
      Big_Add_Finish:
       	...
       	mov [result_len], bh
       	;dec ecx
       	mov byte[ecx], 31h ;  One Bit Carray At Highest Bit
    Recorver:
      mov bh, [result_len]
      pop edx
     	pop ecx
...
section .data
    ...
    fib_cal_temp: times 128 db '0'
    buffer_len equ 128
    fib_cal_result: times 127 db '0'
    e2: db '1'
    result_len : db 1
section .bss
    Input: resb 30
