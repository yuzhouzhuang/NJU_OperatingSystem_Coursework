...
int test9() {
    ...
    if (read(&v1, va, 1) != 0 || read(&v2, va + 1024, 1) == 0) {
        Fail("test9: fail, read");
    }

    if (free(va + 2048, 1) == 0) {
        Fail("test9: fail, free");
    }
...
int test10() {
    ...
    v_address va, vb;
    m_size_t size = 1024;
    if (allocate(&va, size, 1) != 0 || allocate(&vb, size, 1) != 0) {
        Fail("test10: fail, big allocation");
    }

    if (free(va, 1) != 0) {
        Fail("test10: fail, free");
    }

    data_unit v2;

    if (read(&v2, vb, 1) != 0) {
        Fail("test10: fail, read");
    }
